...
with torch.no_grad():
    for i in tqdm(range(len(test))):
        ...
        tree.append(test[i]['tree'])
        #pyg = [graphise(targ[j]['sem'], model.wordencoder, model.tokenizer, device)]
        #graph += pyg 
        image.append(Image.open(imagepath + test[i]['path']))
    ...
    vis_embs = vis_embs / vis_embs.norm(p=2, dim=-1, keepdim=True)
    
    syn_embs, syn_attn = model.syn_encoder(
        syn_seq = syn_seqs, 
        lvl_seq = lvl_seqs, 
        pos_seq = pos_seqs, 
