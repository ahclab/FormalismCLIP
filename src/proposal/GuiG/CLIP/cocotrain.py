    ...
    for epoch in range(epochs):
        ...
        for batch in vloader: 
            ...
            with torch.no_grad(): 
                ...
                wandb.log({'GuiG_COCO_validloss': validloss}) 
                if validloss < best_valid_loss: 
                    best_valid_loss = validloss
                    torch.save(model.state_dict(), 'cocomodels/GuiG_COCO_best.pt')
        ...
        acc = cn / leng * 100 
        print('Accuracy: ', acc) 
        wandb.log({'GuiG_COCO_acc': acc})
        var = torch.var(torch.tensor(losses))
        wandb.log({'GuiG_COCO_var': var}) 
