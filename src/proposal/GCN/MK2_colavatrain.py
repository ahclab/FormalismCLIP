...
def main():
    ...
    for epoch in range(epochs):
        ...
        for batch in tloader: 
            ...
            loss = outputs['loss'] 
            #print('loss: ', loss)
            wandb.log({'MK2_COLAVA_GCN_train_loss': loss})
            loss.backward() 
            optim.step()
            scheduler.step()
            optim.zero_grad()
        ...
        for batch in vloader: 
            ...
            with torch.no_grad(): 
                ...
                wandb.log({'MK2_COLAVA_GCN_valid_loss': validloss})
                
                if validloss < best_valid_loss: 
                    best_valid_loss = validloss 
                    torch.save(model.state_dict(), "colavamodels/MK2_COLAVA_GCN_best.pt")
                    print('saved')
