...
def cocograph(doc, text_encoder, tokenizer, device):
    ...
    for line in edge_list:
        ...
        if attr in node_dict.keys():
            pass 
        else: 
            node_list.append(attr)
            node_dict[attr] = new_node_num
            new_node_num += 1
              
        edge_index.append([line[0], node_dict[attr]])
        edge_index.append([node_dict[attr], line[1]])
...
def main(): 
    ...
    for epoch in range(epochs):
        ...
        for batch in tloader: 
            ...
            loss = outputs['loss'] 
            print("loss: ", loss) 
            wandb.log({'MK2_COCO_GCN_loss': loss})
            loss.backward() 
            optim.step() 
            scheduler.step() 
            optim.zero_grad() 
        ...
        for batch in vloader: 
            ...
            with torch.no_grad():
                ...
                wandb.log({'MK2_COCO_GCN_validloss': validloss})
                
                if validloss < best_valid_loss: 
                    best_valid_loss = validloss
                    torch.save(model.state_dict(), 'MK2_cocomodels/MK2_COCO_GCN_best.pt')
                    print("Model saved")
        ...
        acc = cn / leng * 100
        print("acc: ", acc)
        wandb.log({'MK2_COCO_GCN_acc': acc})
        
        torch.save(model.state_dict(), 'MK2_cocomodels/MK2_COCO_GCN_epoch' + str(epoch + 1) + '.pt')
        var = torch.var(torch.tensor(losses))
        wandb.log({'MK2_COCO_GCN_valid_var': var})
