...
def main():
    ...
    for epoch in range(epochs):
        ...
        for batch in tloader: 
            ...
            loss = outputs['loss'] 
            print('loss: ', loss)
            wandb.log({'K1_wtxt_graphconv_train_loss': loss})
            loss.backward() 
            optim.step()
            scheduler.step()
            optim.zero_grad()
        ...
        for batch in vloader: 
            ...
            with torch.no_grad(): 
                ...
                wandb.log({'K1_wtxt_graphconv_valid_loss': validloss})
                
                if validloss < best_valid_loss: 
                    best_valid_loss = validloss 
                    torch.save(model.state_dict(), "lavamodels/K1_wtxt_graphconv_best.pt")
                    print('saved')
        ...
        acc = cn / leng * 100
        print('acc: ', acc)
        wandb.log({'K1_wtxt_graphconv_acc': acc})
        
        torch.save(model.state_dict(), "lavamodels/K1_wtxt_graphconv_epoch" + str(epoch + 1) + ".pt")
        var = torch.var(torch.tensor(losses))
        wandb.log({'K1_wtxt_graphconv_valid_var': var})
